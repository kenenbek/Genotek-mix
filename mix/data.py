...
class MyOwnDataset10Class(MyOwnDataset):
    ...
    def process(self):
        ...
        for raw_path in self.raw_paths:
            ...
            ibd_list_ = defaultdict(lambda: [[], [], [], [], [],
                                             [], [], [], [], []])
            x_data = defaultdict(lambda: (30 * [0]))
...
class MyOwnDataset10ClassCont(MyOwnDataset):
    ...
    def process(self):
        ...
        for raw_path in self.raw_paths:
            ...
            y_labels = dict(sorted(y_labels.items()))
            y = torch.Tensor(list(y_labels.values())).type(torch.float).contiguous()
